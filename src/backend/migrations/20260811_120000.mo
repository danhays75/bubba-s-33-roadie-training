...
module {
  // Twenty-fifth migration: add two additive fields to FoodRecipe to
  // support detailed prep recipes — `stepGroups : [FoodStepGroup]` for
  // grouped/sectioned procedure steps and `whys : [FoodWhy]` for the
  // "Bubba's Why's" training Q&A.
  //
  //   FoodRecipe.stepGroups : [FoodStepGroup]
  //     FoodStepGroup = { title : ?Text; steps : [Text] }
  //   FoodRecipe.whys : [FoodWhy]
  //     FoodWhy = { question : Text; answer : Text }
  //
  // Both fields are additive and default to empty lists for every existing
  // food recipe. Back-compat: recipes without `stepGroups` (empty list) fall
  // back to the flat `steps` array exactly as today; recipes without `whys`
  // (empty list) omit the "Bubba's Why's" section entirely. The existing
  // flat `steps : [Text]` field is kept for backward compatibility — only
  // the two new arrays are added.
  //
  ...
  // every field is carried forward; only the new `stepGroups` and `whys`
  // fields are added as empty lists. Items without a foodRecipe pass through
  ...
  // (20260811_000000.mo) — the previously deployed stable signature, where
  // FoodRecipe has no `stepGroups` or `whys` fields. The NewActor inlines the
  // same shape plus the two new fields on FoodRecipe.
  ...
  // FoodStepGroup mirrors src/backend/types/library.mo FoodStepGroup. New
  // type introduced by this migration — a named section of grouped
  // procedure steps on a food recipe. `title` is the optional section
  // header; `steps` is the ordered list of step strings for that section.
  type FoodStepGroup = {
    title : ?Text;
    steps : [Text];
  };

  // FoodWhy mirrors src/backend/types/library.mo FoodWhy. New type
  // introduced by this migration — a single "Bubba's Why's" training Q&A
  // entry on a food recipe. `question` is the bolded question; `answer` is
  ...
  // sixteen-field shape with buildHeader, components with amounts, but no
  // stepGroups or whys). Carried forward from the preceding migration's
  // NewFoodRecipe.
  type OldFoodRecipe = {
    station : Text;
    kind : FoodRecipeKind;
  ...
  // old fields unchanged PLUS the new `stepGroups : [FoodStepGroup]` and
  // `whys : [FoodWhy]` fields added as the last fields. Both default to
  // empty lists for every existing food recipe (back-compat: empty
  // stepGroups = fall back to flat `steps`; empty whys = omit the section).
  // The existing flat `steps : [Text]` field is kept for backward
  // compatibility.
  type NewFoodRecipe = {
    station : Text;
    kind : FoodRecipeKind;
  ...
  // (no stepGroups or whys). The nested ?Recipe uses the unchanged Recipe
  // type.
  type OldLibraryItem = {
    id : Nat;
    categoryId : Nat;
  ...
  // (with stepGroups = [] and whys = []). The nested ?Recipe uses the
  ...
  // forward verbatim and adding the new `stepGroups = []` and `whys = []`.
  // Existing food recipes have no grouped procedure steps and no Bubba's
  // Why's Q&A; empty lists mean "fall back to the flat `steps` array and
  // omit the Why's section — render exactly as today" (back-compat for
  // every existing prep and menuBuild recipe).
  ...
  // to add stepGroups = [] and whys = []. The nested ?Recipe (beverage) is
  // carried forward unchanged — the stepGroups/whys addition does not touch
  // beverage recipes. Items without a foodRecipe pass through unchanged.
  ...
  public func migration(old : OldActor) : NewActor {
    ...
    // every field is carried forward; only the new `stepGroups` and `whys`
    // fields are added as empty lists. Items without a foodRecipe are
    // carried forward verbatim (only the foodRecipe-bearing items get the
    // new empty stepGroups and whys on their nested foodRecipe).
