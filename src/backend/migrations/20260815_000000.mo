...
module {
  // Twenty-seventh migration: add an additive `qualityGroups :
  // [FoodQualityGroup]` field to FoodRecipe to support grouped Quality
  // Identifiers on multi-set prep recipes (e.g. Cheese Prep – Parmesan –
  // Blocking & Grating, which has separate Blocked and Grated sets, each
  // with its own Shelf Life / Storage / Storage Temperature).
  //
  //   FoodRecipe.qualityGroups : [FoodQualityGroup]
  //     FoodQualityGroup = { title : ?Text; items : [Text] }
  //
  // The field is additive and defaults to an empty list for every existing
  // food recipe. Back-compat: recipes without `qualityGroups` (empty list)
  // fall back to the flat `qualityIdentifiers` array exactly as today —
  // single-set prep recipes and all menuBuild recipes are unchanged. The
  // existing flat `qualityIdentifiers : [Text]` field is kept for backward
  // compatibility; only the new `qualityGroups` array is added.
  //
  ...
  // every field is carried forward; only the new `qualityGroups` field is
  // added as an empty list. Items without a foodRecipe pass through
  // unchanged. Categories, positions, and all other stable state are carried
  // forward UNCHANGED (this migration touches only the items list, and only
  // the foodRecipe-bearing items within it).
  //
  // OldActor mirrors the NewActor of the preceding migration
  // (20260811_130000.mo) — the previously deployed stable signature, where
  // FoodRecipe has stepGroups, whys, and yields but no `qualityGroups` field.
  // The NewActor inlines the same shape plus the new `qualityGroups` field on
  // FoodRecipe.
  ...
  // shape). Used as the element type of the `yields` field on FoodRecipe.
  // Inlined here as its own type alias for clarity, matching the source's
  // distinct naming of the per-size yield concept.
  type FoodSizeAmount = {
    size : Text;
    value : Text;
  };
  ...
  // FoodQualityGroup mirrors src/backend/types/library.mo FoodQualityGroup.
  // New type introduced by this migration — a single grouped set of Quality
  // Identifiers on a food recipe. `title` is the set name (e.g. "Blocked
  // Parmesan Cheese"), or null for a shared/general block that precedes the
  // named sets; `items` is that set's full identifier lines verbatim
  // (including its own "Shelf Life:" / "Storage:" / "Storage Temperature:" /
  // "Line Holding:" lines).
  type FoodQualityGroup = {
    title : ?Text;
    items : [Text];
  };

  // OldFoodRecipe mirrors the PRE-migration FoodRecipe shape (the
  // nineteen-field shape with stepGroups, whys, and yields, but no
  // qualityGroups). Carried forward from the preceding migration's
  // NewFoodRecipe.
  type OldFoodRecipe = {
    station : Text;
    kind : FoodRecipeKind;
  ...
  // NewFoodRecipe mirrors the POST-migration FoodRecipe shape — the nineteen
  // old fields unchanged PLUS the new `qualityGroups : [FoodQualityGroup]`
  // field added as the last field. qualityGroups defaults to an empty list
  // for every existing food recipe (back-compat: empty qualityGroups = fall
  // back to the flat `qualityIdentifiers` array; single-set prep recipes and
  // all menuBuild recipes are unchanged). The existing flat
  // `qualityIdentifiers : [Text]` field is kept for backward compatibility.
  type NewFoodRecipe = {
    station : Text;
    kind : FoodRecipeKind;
    menuSection : ?Text;
  ...
  // (no qualityGroups). The nested ?Recipe uses the unchanged Recipe type.
  type OldLibraryItem = {
    id : Nat;
    categoryId : Nat;
    title : Text;
  ...
  // (with qualityGroups = []). The nested ?Recipe uses the unchanged Recipe
  // type.
  type NewLibraryItem = {
    id : Nat;
    categoryId : Nat;
  ...
  // forward verbatim and adding the new `qualityGroups = []`. Existing food
  // recipes have no grouped Quality Identifiers; an empty list means "fall
  // back to the flat `qualityIdentifiers` array — render exactly as today"
  // (back-compat for single-set prep recipes and all menuBuild recipes).
  ...
  // to add qualityGroups = []. The nested ?Recipe (beverage) is carried
  // forward unchanged — the qualityGroups addition does not touch beverage
  // recipes. Items without a foodRecipe pass through unchanged.
  ...
  public func migration(old : OldActor) : NewActor {
    ...
    // every field is carried forward; only the new `qualityGroups` field is
    // added as an empty list. Items without a foodRecipe are carried forward
    // verbatim (only the foodRecipe-bearing items get the new empty
    // qualityGroups on their nested foodRecipe).
